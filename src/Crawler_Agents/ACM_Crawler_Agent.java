...
public class ACM_Crawler_Agent {
	...
	public ACM_Crawler_Agent(Main App) {
...
//------------------------------------------------------------------------------------------------------------------------------
						...
						for (Element article : articlesFullDoc) {
							...
							Elements keywords = doc_Article.select("ol[class=rlist level-3 ch-1 hasNodes]").select("div");
...
//---------------------------------------------------- Geting Authors IMGS + DATA ------------------------------------------------------------------------						
							for (int k = 0; k < article.select("ul").get(0).select("img[class=author-picture]")
								...
								Author Author = new Author(article.select("ul").get(0).select("a").get(k).attr("title"),url.getPath());
								...
								if (!default_profile_img.equals(article.select("ul").get(0)
									...
									downloadFile(url, "UsersPNG\\"
											+ article.select("ul").get(0).select("a").get(k).attr("title") + ".jpg");
